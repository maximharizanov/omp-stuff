...
const REVIEW_REQUEST_TEMPLATE = `## Code Review Request
...
interface BuildReviewPromptOptions {
	reviewRoot?: string;
}
...
function buildReviewPrompt(
	...
	if (typeof renderPromptTemplate === "function") {
		return renderPromptTemplate(REVIEW_REQUEST_TEMPLATE, {
			mode,
	...
	const distributionGuidelines = agentCount === 1
		...
		: [
			`Spawn **${agentCount} reviewer agents** in parallel.`,
			"",
			"Group files by locality, e.g.:",
			"- Same directory/module → same agent",
			"- Related functionality → same agent",
...
async function getTemporaryPrWorktreePath(repoRoot: string, prRef: string): Promise<string> {
	return path.join(
		...
		`${path.basename(repoRoot)}-review-${sanitizeWorktreeSegment(prRef)}-${Date.now()}`,
	);
}
		...
		async execute(_args: string[], ctx: {
			...
			switch (modeNum) {
				case 1: {
					...
					return buildReviewPrompt(api, `Reviewing changes between \`${baseBranch}\` and \`${currentBranch}\` (PR-style)`, stats, diffResult.stdout);
				}
				...
				case 2: {
					...
					return buildReviewPrompt(api, "Reviewing uncommitted changes (staged + unstaged)", stats, combinedDiff);
				}
				...
				case 3: {
					...
					return buildReviewPrompt(api, `Reviewing commit \`${hash}\``, stats, showResult.stdout);
				}
				...
				case 5: {
					...
					if (hasDiff) {
						const stats = parseDiff(diffResult.stdout);
						return `${buildReviewPrompt(api, `Custom review: ${instructions.split("\n")[0].slice(0, 60)}…`, stats, diffResult.stdout)}\n\n### Additional Instructions\n\n${instructions}`;
					}
